...
![](https://static.scientificamerican.com/blogs/cache/file/5687448D-1F52-4287-A13D53F37A35BE41_source.jpg){width=50%}
...
![](https://i.ytimg.com/vi/A-L7KOjyDrE/maxresdefault.jpg){width=50%}
...
The main reason that Bayesian statistics were not widely adopted initially is probably a practical one. It boils down to $P(data|model)$, the likelihood, which we need to calculate to estimate $P(model|data)$, the posterior. you may have heard of "maximum likelihood" methods. Maximum likelihood algorithms are used to fit frequentist models. They try to find the set of parameters that make the data the most probable. In the Bayesian world, we are still trying to do that, but instead of getting a single point estimate of the parameters that are most likely given the data, we are now trying to estimate a joint probability distribution over all possible combinations of values the parameters could take. We end up with a *distribution* for all the parameters that is most likely given the data we observed.
...
![](https://mc-stan.org/images/stan_logo.png){width=25%}
...
It's hard to visually separate the fields, because there are 25 of them and they largely overlap. Let's add different regression lines for each field. This is equivalent to fitting a separate linear model to each field. 
...
You can see that the relationship between yield and soil nitrogen varies by field -- but it's positive on average. A multilevel model helps us estimate the overall trend while taking into account the variation in trend (slope and intercept) between fields.
...
- Here the only fixed effect is the global intercept (`1`) and there is a random intercept fit to each field. 
- The random effects specification (`(1 | field)`) has a *design* side (on the left hand) and *group* side (on the right hand) separated by `|`. 
- In this case, the `1` on the design side means only fit random intercepts and no random slopes.
- The `field` on the group side means each field will get its own random intercept.
...
We see low numbers for `Rhat`. Looking at the parameter values, we can see the fixed-effect or "population-level" intercept is about 5.9, with a 95% credible interval ranging from 5.6 to 6.3. We can also see the standard deviation for the "group-level" or random field intercepts `sd(Intercept)` and for the plots within fields (the standard deviation of the overall model residuals `sigma`).
...
It is a good idea to examine the proportions of variance at different levels in your nested dataset. In this case we can calculate the ratio of variance within to between fields. This is made easy by the `variance_decomposition()` from the [**performance**](https://easystats.github.io/performance/) package (part of [**easystats**](https://easystats.github.io/easystats/)), which also gives us a credible interval. We'll use 99%.

```{r}
variance_decomposition(fit_interceptonly_moresamples, ci = 0.99)
```

This shows that the proportion of variance within fields is much greater than zero meaning there's a strong need for a multilevel model in this case. Actually, if you have a dataset with a grouped structure, I would recommend still using a multilevel model even if the variance decomposition does not necessarily say it's warranted. It's better to stick with the model that your study design calls for.

## Mixed-effects model with first level predictors 

The intercept-only model is a good start. But so far all we've done is pull out the random variation in mean yield by field. We want to make inference about what factors influence yield at the individual plot level. To do this, we can add some fixed effect predictors (slopes) to the model.

We'll start with the first-level predictors that vary by plot. We will add a fixed effect for variety and for soil N. Because these slopes are fixed and not random, that means we are assuming that the effects of variety and soil N on yield are the same in each field. Also, note we still haven't specified our own priors, so **brms** is defaulting to very wide and uninformative priors.
...
`summary()` shows low R-hat statistics and also shows that most of the mass of the posterior distribution for the slopes is well above zero. This indicates a strong positive effect of variety (the model codes short=0 and tall=1, meaning the tall variety has higher yield on average) and a strong positive effect of soil N on yield. We have roughly the same random effect variance parameter as before, but now `sigma` (the standard deviation of the residuals for individual plots) is a lot lower than before. This makes sense because we have some effects in the model that explain some of the variance and result in lower residuals.
...
Variety and soil N are first-level predictors: they vary by individual plot, even those that share the same field. A second-level predictor is one that is shared by all plots in a field. In this dataset, the `rainfall` variable is a second-level predictor because all plots in the same field have the same rainfall value. This would happen if, for example, only one rain gauge is installed per field to measure rainfall. 

It does not require any different syntax to add a second-level predictor to the model. It is put in the model formula in exactly the same way as the first-level predictors. Here is another model with `rainfall` added as an additional fixed effect slope.
...
We have strong evidence for a positive effect of rainfall on yield at the field level. 

## Interactions between fixed effects

We can also add any interaction terms we would like if we want to test whether any of the fixed effects interact. For example, is the effect of soil nitrogen on yield stronger for the short or tall variety? That would be an interaction between the two first-level terms `variety` and `soilN`. Or is the effect of soil N on yield stronger in fields with higher rainfall? That would be a cross-level interaction between the first-level predictor `soilN` and the second-level predictor `rainfall``. 

Regardless of what level the predictors are, you can specify the interaction between fixed effects `a` and `b` by adding `a:b` to the fixed-effect part of the model formula. For example, if you want to model the main effects of variety, soil N, and rainfall, and add an interaction between soil N and rainfall, the model formula would include `1 + variety + soilN + rainfall + soilN:rainfall`.

I will leave the interaction model for you to fit as an exercise.

## Mixed-effects model with random slopes

So far, we have assumed that any effect of the predictor variables is constant across all the fields. We've allowed the intercept for yield to be different from field to field, but not the slope with respect to any of the predictor variables. We can add *random slopes* terms to the model to allow not only the intercept, but also the slope, of the yield-soil N relationship to vary by field. 

To specify random slopes, we modify the *design* side of the random-effect term. For our random intercept model, we used `(1 | field)`. To fit a random slope for each field with respect to soil N, we would use `(1 + soilN | field)`. To fit a random slope for both soil N and variety, we would use `(1 + variety + soilN | field)`.

Let's go ahead and try to include random slopes for both the first-level predictors, `variety` and `soilN`. Note that it doesn't make sense to have a random slope for `rainfall` that differs by field, because `rainfall` is the same for all plots in a field. We can't estimate a slope of rainfall versus soil N within an individual field, only across all the fields.

> Note: this model may take a little longer to sample than the previous ones (~1 minute on my laptop, after compilation). That's because it has quite a few more random effect parameters and I've also added more warmup iterations to make sure the model converges.
...
Notice you may get a warning message that there were "divergent transitions after warmup." The [linked documentation page](http://mc-stan.org/misc/warnings.html#divergent-transitions-after-warmup) gives more information about what this means. Essentially, it is a problem with the HMC sampler that is often caused by trying to fit an overly complicated model (i.e., one with too many parameters) to a too-small dataset. We will see in a moment why that might be the case here.
...
You can see from the summary that the standard deviation of the random slope for variety is small relative to its uncertainty (point estimate of 0.14, lower bound of 95% credible interval 0.01). Given that the fixed component of the slope for variety is estimated to be about 1.27, we can likely omit the random slope for variety from the model without losing much explanatory power.

To confirm this, plot the posterior credible intervals of the field-level slopes for variety (sum of fixed and random components). The black dots are the medians.
...
This plot is not really possible to make in a frequentist analysis (unless you tried some fancy bootstrapping techniques) but it is easy as pie in a Bayesian context. As you can see from the plot, although we allowed the relationship between variety and yield to vary from class to class, the model found that the field-level error was fairly low. All the slope medians are between 1.2 and 1.4. Therefore we will try to refit the model without the random slope for variety (but retaining the one for soil nitrogen).
...
We see that the fit with only the soil N random slope is ranked best in our comparison. The model with both random slopes has a difference of 0.3 ELPD from the best model, but the standard error of that difference is 1.6. This is basically a "tie" so we can safely go with the simpler of the two: `fit_soilNrandomslope`. The models without any random slopes perform far worse (by about 6 log units), and the intercept-only model is a very bad fit (not surprisingly).
...
```{r}
conditional_effects(fit_soilNrandomslope)
```

We can also incorporate the random effects into the prediction using `conditional_effects()`, as in this example of field-level fitted relationships between yield and soil N (with a few plotting options added to make the plot more readable):
...
If you compare this plot to the scatterplot we made earlier with a separate simple regression trendline fit to the yield versus soil N data from each field, you might notice the lines on this plot have slightly less extreme slopes. That illustrates what's called "shrinkage": the mixed model is adjusting for the overestimation of trends we would have if we fit separate models for each field. The slopes "shrink" toward the mean overall slope. Check out this [animated demo of shrinkage](https://m-clark.github.io/posts/2019-05-14-shrinkage-in-mixed-models/) for more details!
...
Or use the function `add_epred_draws()` to get posterior expected values at specified values of the fixed effects and plot them. For instance here's a plot of the model prediction for the trend of yield versus soil N for each variety, at the average value of rainfall, and averaging across the random variation of fields (so these are the population-level expectations).
...
As you can see we had no interaction term for variety and soil N in the model so the slopes are the same for both varieties, but the tall variety has a higher intercept (greater yield on average).
...
These three exercises should help you test your understanding of Bayesian statistics and using **brms**. Answers follow the exercises.

### Exercise 1

![](https://qdread.github.io/SEAStats/images/werewolf.jpg){width=25%}  
...
fitbetter <- brm(y ~ x1 + (1+x1|g1) + (1+x1|g2), data = a, chains = 4, warmup = 2000, iter = 3000, seed = 1, file = 'ex2good')
```

```{r, echo = FALSE}
plot(fitbetter, variable = 'b_x1')
```

#### Trace plot B

```{r, echo = FALSE}
plot(fit, variable = 'b_x1')
```


### Exercise 3

Add one or more interaction terms to the model that we have been working with. For example you could test for the presence of a variety-soil N interaction, or a rainfall-soil N interaction. Examine the model diagnostics, show or plot the parameter estimates, and use LOO cross-validation to compare this model to the ones we fit previously. What can you conclude?

#### Exercise 3 hints

- To include the variety-soil N interaction, add the term `+ variety:soilN` to the fixed-effects part of the model formula. 
- To include the rainfall-soil N interaction, add the term `+ rainfall:soilN` to the fixed-effects part of the model formula.
