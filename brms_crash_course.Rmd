...
- write some exercises (not necessary for now)
- clean up headings
-->
...
- `pupil`: a numeric ID for each individual (1-26). Note that the IDs are nested, meaning the same ID is reused from class to class. For example pupil 1 in class 1 is not the same person as pupil 1 in class 2.
...
We can make more customized plots of the fixed effects, with more than one credible interval, if we generate estimated marginal means and trends (posterior predictive distributions) using the **emmeans** package and plot them using the **tidybayes** package.
