...
The main reason that Bayesian statistics were not widely adopted initially is probably a practical reason. It boils down to $$P(data|model)$$, the likelihood. If our "model" is just a single parameter like the probability of a coin flip being heads, the calculations are trivial. But what if our model is a multilevel regression model with a bunch of fixed effects, a bunch of random intercepts at multiple levels, and variance-covariance parameters for all the errors (like we are about to fit in this very lesson)? We might have hundreds of parameters. To estimate $$P(data|model)$$, we have to integrate a probability density function across n-dimensional space, where n is the total number of parameters in the model! 
...
That's where Markov Chain Monte Carlo (MCMC), aided by super-fast modern microprocessors, comes in. What is MCMC?
...
You might be asking yourself, if it's so hard to do Bayesian inference, why bother? Maximum likelihood frequentist methods work really well! 
