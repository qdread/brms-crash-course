...
In its simplest form, Bayesian inference is a 
...
The main reason that Bayesian statistics was not widely adopted initially is probably a practical reason. It boils down to `p(data|model)`, the likelihood. This is a 
...
```{r}
popular2data <- readRDS(gzcon(url('https://github.com/qdread/brms-crash-course/raw/main/data/popular2data.rds')))
```
